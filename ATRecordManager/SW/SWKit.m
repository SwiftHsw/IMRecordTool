...
- (UIImage *)resizeImageToSize:(CGSize)size image:(UIImage *)image {
    return [self resizeImageToSize:size opaque:NO scale:0 image:image];
}

- (UIImage *)resizeImageToSize:(CGSize)size opaque:(BOOL)opaque scale:(CGFloat)scale image:(UIImage *)image{
...
+ (UIImageView *)getAccessoryImage{
    ...
    pushImage.image = [UIImage imageNamed:@"acount_btn_go"];
    pushImage.frame = CGRectMake(0, 0, 9, 16);
    return pushImage;
}
